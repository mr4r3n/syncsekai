...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token.trim()) {
      showToast(t('auth.enterRecoveryToken'), 'error');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!tokenParam && (
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-[var(--text-secondary)]">{t('auth.recoveryCodeOrToken')}</label>
              <div className="flex items-center gap-3 px-3.5 py-2.5 rounded-[6px] border border-[var(--border-subtle)] bg-[var(--bg-surface)] backdrop-blur-md focus-within:border-[#FF634A] focus-within:ring-1 focus-within:ring-[#FF634A] transition-colors">
                <KeyRound className="w-4 h-4 text-[var(--text-muted)] shrink-0" />
                <input
                  type="text"
                  value={token}
                  onChange={(e) => setToken(e.target.value)}
                  required
                  placeholder={t('auth.pasteRecoveryCode')}
                  className="w-full bg-transparent text-sm text-[var(--text-primary)] outline-none font-mono"
                />
              </div>
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading || !isLengthValid || !isMatch || !token.trim()}
...
export default function ResetPasswordRootPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-app)] flex flex-col items-center justify-center p-4 selection:bg-[#FF634A]/30 relative overflow-hidden">
      ...
      <Suspense fallback={<div className="text-xs text-[var(--text-muted)] font-mono">{t('auth.loadingForm')}</div>}>
        <ResetPasswordForm />
      </Suspense>
    </div>
  );
