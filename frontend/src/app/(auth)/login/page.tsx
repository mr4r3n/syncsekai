...
export default function LoginPage() {
  ...
  const [twoFactorType, setTwoFactorType] = useState<'APP_TOTP' | 'EMAIL_OTP'>('APP_TOTP');
    ...
    if (typeof window !== 'undefined') {
      ...
      if (expired === 'true') {
        ...
        if (error.includes('NOT_CONFIGURED')) {
          ...
          showToast(`${t('auth.socialError')} ` + error, 'error');
        }
      }
      ...
      api.auth
        .me()
        .then((me) => {
          if (me && (me.id || me.user?.id || me.username)) router.replace('/connections');
        })
        .catch(() => {});
    }
  }, []);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!email.trim() || !password) {
      const msg = t('auth.fillEmailPassword');
      setFormError(msg);
      showToast(msg, 'error');
      return;
    }
    ...
    try {
      ...
      if (res.requires2FA) {
        setRequires2FA(true);
        setTwoFactorType(res.twoFactorType || 'APP_TOTP');
        showToast(res.message || t('auth.twoFactorRequired'), 'info');
        setLoading(false);
        return;
      }
    ...
    >
        ...
        <div className="my-auto py-10 space-y-8 relative z-10 max-w-xl">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono text-emerald-400 bg-emerald-500/10 border border-emerald-500/20">
              ...
              <span>{t('auth.asideTitle')}</span>
            </div>
            <h2 className="text-3xl xl:text-4xl font-extrabold tracking-tight font-heading leading-tight text-[var(--text-primary)]">{t('auth.asideHeadline')}</h2>
            <p className="text-sm text-[var(--text-secondary)] leading-relaxed">{t('auth.asideDesc')}</p>
          </div>

          {/* SERVICIOS COMPATIBLES EN FORMATO MINIMALISTA */}
          <div className="space-y-3 pt-2">
            <div className="flex items-start gap-3 p-3.5 rounded-[8px] bg-[var(--bg-surface-elevated)] border border-[var(--border-subtle)] backdrop-blur-md">
              <div className="w-8 h-8 rounded-[6px] bg-[var(--brand-plex)]/10 text-[var(--brand-plex)] flex items-center justify-center shrink-0 mt-0.5">
                <Tv className="w-4 h-4" />
              </div>
              <div className="space-y-0.5 text-xs">
                <div className="font-bold text-[var(--text-primary)]">Plex, Jellyfin &amp; Emby</div>
                <div className="text-[11px] text-[var(--text-secondary)]">{t('auth.asideFeatureNative')}</div>
              </div>
            </div>

            <div className="flex items-start gap-3 p-3.5 rounded-[8px] bg-[var(--bg-surface-elevated)] border border-[var(--border-subtle)] backdrop-blur-md">
              <div className="w-8 h-8 rounded-[6px] bg-sky-500/10 text-sky-400 flex items-center justify-center shrink-0 mt-0.5">
                <Layers className="w-4 h-4" />
              </div>
              <div className="space-y-0.5 text-xs">
                <div className="font-bold text-[var(--text-primary)]">AniList, MAL &amp; Kitsu</div>
                <div className="text-[11px] text-[var(--text-secondary)]">{t('auth.asideFeatureDual')}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="col-span-1 lg:col-span-6 xl:col-span-5 flex flex-col justify-between p-6 sm:p-10 xl:p-14 min-h-screen relative z-10">
        ...
        <main id="main-content" tabIndex={-1} className="w-full max-w-md mx-auto my-auto py-8 space-y-7 outline-none">
          ...
          {requires2FA ? (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label htmlFor="otp" className="text-xs font-semibold text-[var(--text-secondary)]">{t('auth.otpLabel')}</label>
                <div
                  suppressHydrationWarning
                ...
                >
                  <KeyRound className="w-4 h-4 text-[var(--text-muted)] shrink-0" aria-hidden="true" />
                  <input
                    suppressHydrationWarning
                    id="otp"
                    name="otp"
                    ...
                    placeholder="123456"
                    className="w-full bg-transparent text-sm font-mono tracking-widest text-[var(--text-primary)] outline-none"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 pt-1">
                <button
                  ...
                  className="px-4 py-3 rounded-[6px] border border-[var(--border-subtle)] bg-[var(--bg-surface-elevated)] backdrop-blur-md text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)] transition-colors flex items-center gap-1.5 cursor-pointer"
                >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <svg className="w-4 h-4 shrink-0 fill-[var(--brand-discord)]" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <div className="relative flex items-center justify-center pt-2">
                  ...
                  <span className="bg-[var(--bg-app)] px-3 text-[11px] font-mono text-[var(--text-muted)] uppercase tracking-wider shrink-0">
                ...
                <div className="space-y-1.5" suppressHydrationWarning>
                  <div className="flex items-center justify-between">
                    ...
                    <Link
                      ...
                      className="text-[11px] font-medium text-[var(--accent-text)] hover:underline hover:text-[var(--accent-primary-hover)] transition-colors"
                    >
          ...
          {/* Enlace a Registro */}
          <div className="pt-4 text-center text-xs text-[var(--text-secondary)] border-t border-[var(--border-subtle)]" suppressHydrationWarning>
